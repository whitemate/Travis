...
public class CSV2RDF implements Runnable {
	...
	public void run() {
		...
		try {
			try (Reader in = Files.newReader(inputFile, INPUT_CHARSET)){
				try (CSVReader reader = new CSVReader(in, toChar(separator), toChar(quote), toChar(escape))){
					String[] row = reader.readNext();

					Preconditions.checkNotNull(row, "Input file is empty!");

					try(Writer out = Files.newWriter(outputFile, OUTPUT_CHARSET)){
		 ...
		 * FIXED!
		 */
		LOGGER.log(Level.INFO,() -> "Converted " + inputRows + " rows to " + outputTriples + " triples%n");
	...
	private class Template {
		...
		Template(List<String> cols, File templateFile, RDFWriter writer){
		...
		private String insertPlaceholders(List<String> cols, File templateFile){
		...
		private void parseTemplate(List<String> cols, File templateFile, final RDFWriter writer) {
			...
			parseData(templateStr, parser);

		}

		private void parseData(String templateStr, RDFParser parser) {
			try {
	...
	public static void main(String[] args) {
		try {
			...
			/**
			 * Vulnerability and CodeSmell fixed:
			 */
